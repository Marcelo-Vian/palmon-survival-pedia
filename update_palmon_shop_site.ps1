...
try {
    ...
    if (Test-Path $PrintsDir) {
        $LatestPrint = Get-ChildItem -Path $PrintsDir -File | Where-Object { $_.Extension -match "png|jpg|jpeg" } | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    ...
    git add .gitignore ATUALIZAR_SITE_LOJA_1_CLIQUE.bat COMO_ATUALIZAR_SITE_LOJA.md update_palmon_shop_site.ps1 build_shop_capture_site.py generate_shop_offer_thumbnails.py generate_shop_home_preview.py palmon_shop_captures.html shop_analyzer_screenshot_wide.png shop_active_offers_*.json assets/shop_offer_thumbs
